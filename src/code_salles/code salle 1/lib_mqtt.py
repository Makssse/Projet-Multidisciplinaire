...
class MQTTClient:
    ...
    def connecter(self):
        ...
        try:
            self.sock.connect(addr)          # addr = (host, port)
            print("Connexion établie")
        except OSError as e:
            # Mapping des errno vers un message lisible
            if e.errno == errno.ECONNREFUSED:
                msg = "Connexion refusée – le serveur n’écoute pas sur le port."
            elif e.errno == errno.ETIMEDOUT:
                msg = "Délai d’attente dépassé – le serveur ne répond pas."
            elif e.errno == errno.EHOSTUNREACH:
                msg = "Hôte injoignable – adresse IP incorrecte ou route manquante."
            elif e.errno == errno.EADDRINUSE:
                msg = "Port local déjà utilisé – choisissez un autre port."
            elif e.errno == errno.EINPROGRESS:
                msg = "Connexion en cours (socket non bloquant)."
            else:
...
def connecter_ethernet(ip, mac_str):
    ...
    nic.active(True)
    nic.ifconfig((ip, "255.255.255.0", "192.168.2.1", "192.168.1.1")) # Ligne à modifier selon réseau local
